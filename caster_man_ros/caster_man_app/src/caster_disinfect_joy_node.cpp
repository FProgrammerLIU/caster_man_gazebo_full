...
bool move_goal_flag, init_pose, all_goal_cancel, stop = false;
ros::Publisher init_pub;
...
void GoalCancelPub(int goal_id) {
  actionlib_msgs::GoalID goal_id_cancel;
  goal_id_cancel.id = goal_id;
  pub_stop.publish(goal_id_cancel);
  all_goal_cancel = true;
}
...
bool GoHome(caster_man_app::PickGift::Request &req, caster_man_app::PickGift::Response &res) {
  GoalCancelPub(1);
  ROS_INFO("Go Home");
...
int main(int argc, char** argv) {
  ...
  ros::ServiceServer service_home = nh.advertiseService("gohome", GoHome);
  pub_stop = nh.advertise<actionlib_msgs::GoalID>("move_goal_cancel", 1000);
